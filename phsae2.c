...
//	FILE *source, *intermediate, *opcode, *symbolTable; // Creats the Files pointer
	...
	do
	{
		...
		if(line[0] == '\n'){break;} //Exits when "NEW LINE" has been found Prevent Errors	
		for(int i = 0; i < NUMELEM(ops); ++i) // comparing The first token found to see if its a mnemonic RUNTIME is slow sorrrrrrrrrry
		{
			if(strcmp(token, ops[i].OP) == 0){ //If a mnemoinc is found -> no label 
				labelFound = false; 
				break;
			}
		}
		if(labelFound)
		{
			...
			token = strtok(NULL, " \t"); // gets next word on the line
		...
		else printf("LMAO THIS IS NOT GOING TO COPY");
